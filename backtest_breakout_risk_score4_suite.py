...
# ============================================================
# SKYNET BREAKOUT RISK SCORE4 SUITE
#
# Goal:
# - Find filters that cut "MFE≈0 / fake impulse / bull-trap" trades
# - WITHOUT killing trade count like too-strict top20 filters.
#
# Tests:
# - trend030 vs trend040
# - score>=5 / initiative / pc<=0.55 / vol<=25
# - breakout risk filters:
#     red volume dominance
#     rejection candles
#     false breakouts
#     upper wick pressure
#     recent flush
#     EMA9 slope / close vs EMA9
# - ATR5_B exit from previous best dynamic SL/TP suite
# - max_open 1/2
#
# Does NOT trade.
...
OUT_DIR = ROOT / "data" / "backtest" / "breakout_risk_suite"

ENTRY_CASES = [
    {
        "entry_case": "TREND040_PC055_VOL25_SCORE4_INIT",
        "min_score": 4,
        "need_initiative": 1,
        "max_pc": 0.55,
        "max_vol": 25.0,
    ...
    {
        "entry_case": "TREND030_PC055_VOL25_SCORE4_INIT",
        "min_score": 4,
        "need_initiative": 1,
        "max_pc": 0.55,
        "max_vol": 25.0,
    ...
    {
        "entry_case": "TREND040_PC065_VOL35_SCORE4_INIT",
        "min_score": 4,
        "need_initiative": 1,
        "max_pc": 0.65,
        "max_vol": 35.0,
    ...
    {
        "entry_case": "TREND030_PC065_VOL35_SCORE4_INIT",
        "min_score": 4,
        "need_initiative": 1,
        "max_pc": 0.65,
        "max_vol": 35.0,
...
def candle_stats(row):
    ...
    if min(o, h, l, c) <= 0 or full <= 0:
        return {
            ...
            "range_pct": 0.0, "turnover": quote_turnover(row),
        }
...
def simulate_atr5b(bt, rows, entry_i):
    ...
    for j in range(entry_i + 1, min(entry_i + int(p["time"]) + 1, len(rows))):
        ...
        # Conservative intrabar ordering: full SL before partial.
...
def compute_breakout_metrics(rows, signal_i):
    """
    Uses only candles up to signal_i inclusive.
    These are closed candles in the historical model, so this is live-implementable.
    """
    if signal_i < 2:
        return {}
    ...
    stats15 = [candle_stats(r) for r in lb15]
    stats30 = [candle_stats(r) for r in lb30]

    total_turnover = sum(s["turnover"] for s in stats10)
    red_turnover = sum(s["turnover"] for s, r in zip(stats10, lb10) if s["is_red"])
    ...
    weak_close_count_10m = sum(
        1 for s in stats10
        if s["close_position"] <= 0.45
    )

    upper_wick_pressure_10m = sum(s["upper_wick_ratio"] for s in stats10) / max(1, len(stats10))

    # False breakout proxy:
    # a candle takes local high of prior 5 candles but closes back in lower 60% of its own range.
    ...
    for r in lb15:
        ...
        if o > 0 and c > 0:
            # Large downside sweep from open to low.
            if ((l - o) / o) * 100.0 <= -0.60:
                recent_flush_15m = 1
                break
    ...
    if len(ema9_vals) >= 4:
        close_now = closes30[-1]
        ema9_now = ema9_vals[-1]
        ema9_prev3 = ema9_vals[-4]
        close_vs_ema9_pct = pct(close_now, ema9_now)
        ema9_slope_3m_pct = pct(ema9_now, ema9_prev3)
    else:
...
def passes_risk_mode(metrics, mode):
    if mode == "NO_FILTER":
        return True

    risk = metrics.get("breakout_risk_score", 999)
    red_share = metrics.get("red_volume_share_10m", 1.0)
    reject = metrics.get("rejection_count_10m", 99)
    false_bo = metrics.get("false_breakouts_15m", 99)
    flush = metrics.get("recent_flush_15m", 1)
    ema_slope = metrics.get("ema9_slope_3m_pct", -999.0)
    close_vs_ema = metrics.get("close_vs_ema9_pct", -999.0)
    wick_pressure = metrics.get("upper_wick_pressure_10m", 999.0)
    ...
    if mode == "RISK_LE3":
        return risk <= 3
    if mode == "NO_RED_DOM":
        return red_share < 0.60
    if mode == "NO_REJECTION_CLUSTER":
        return reject <= 1
    if mode == "NO_FALSE_BREAKOUT":
        return false_bo <= 1
    if mode == "NO_RECENT_FLUSH":
        return flush == 0
    if mode == "EMA_OK":
        return ema_slope >= 0.0 and close_vs_ema >= -0.05
    if mode == "COMBO_SAFE":
        return (
            risk <= 2
            and red_share < 0.60
    ...
    if mode == "COMBO_BALANCED":
        return (
            ...
            and reject <= 2
            and false_bo <= 1
            and wick_pressure < 0.38
            and ema_slope >= -0.02
            and close_vs_ema >= -0.10
        )
...
def apply_entry_case(bt, rows, raw, entry_case, confirm_mode):
    ...
    confirm_move_pct = bt.pct(rows[entry_i]["c"], sig_close)
...
async def run(args):
    ...
    risk_modes = parse_list_str(args.risk_modes) or RISK_MODES

    print(f"🚀 SKYNET BREAKOUT RISK SCORE4 SUITE | days={args.days} top={args.top} max_open={max_open_values}")
    print(f"Costs: spread={bt.SPREAD_BPS}bps slippage={bt.SLIPPAGE_BPS}bps")
    print(f"Confirm modes: {confirm_modes}")
    print(f"Risk modes: {risk_modes}")
    ...
    for symbol, raw_signals in raw_signals_by_symbol.items():
        ...
        for raw in raw_signals:
            metrics = compute_breakout_metrics(rows, raw["i"])
            if not metrics:
                continue
            ...
            for entry_case in ENTRY_CASES:
                for confirm_mode in confirm_modes:
                    ...
                    for risk_mode in risk_modes:
                        if not passes_risk_mode(metrics, risk_mode):
                            continue
                        ...
                        key = (entry_case["entry_case"], risk_mode, confirm_mode)
                        records_by_key.setdefault(key, []).append(rec)

    for (entry_case, risk_mode, confirm_mode), records in records_by_key.items():
    ...
    run_dir = OUT_DIR / f"breakout_risk_{stamp}"
    run_dir.mkdir(parents=True, exist_ok=True)

    summary_path = run_dir / f"breakout_risk_summary_{stamp}.csv"
    trades_path = run_dir / f"breakout_risk_trades_{stamp}.csv"
    manifest_path = run_dir / "manifest.json"
    zip_path = run_dir / f"BREAKOUT_RISK_RESULTS_{stamp}.zip"

    summary_fields = [
        "entry_case", "risk_mode", "confirm_mode", "exit_mode", "max_open", "days", "top", "spread_bps", "slippage_bps",
    ...
    trade_fields = [
        "entry_case", "risk_mode", "confirm_mode", "exit_mode", "max_open", "time", "symbol",
        "score", "price_change", "vol_ratio", "trend15", "btc5", "structure", "initiative",
        "confirm_delay_min", "confirm_move_pct", "confirm_min_low",
        "red_volume_share_10m", "rejection_count_10m", "weak_close_count_10m",
        "upper_wick_pressure_10m", "false_breakouts_15m", "recent_flush_15m",
        "close_vs_ema9_pct", "ema9_slope_3m_pct", "breakout_risk_score",
    ...
    for r in all_summary[:40]:
        print(
            f"{r['entry_case']:<36} {r['risk_mode']:<22} {r['confirm_mode']:<16} MO:{r['max_open']} "
    ...
    if args.send_tg:
        caption = (
            f"📦 SKYNET BREAKOUT RISK SCORE4 SUITE\n"
...
def main():
    ...
    p.add_argument("--risk-modes", default=",".join(RISK_MODES))
